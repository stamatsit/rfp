          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {!isLoading && answers.length > 0 && (
                <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight">{answers.length} approved answer{answers.length !== 1 ? "s" : ""}</p>
              )}
            </div>
          </div>
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-700/60">
              {answers.map((item) => (
                <div key={item.id} className={`px-4 py-2.5 transition-colors ${expandedId === item.id ? "bg-emerald-50/40 dark:bg-emerald-900/10" : "hover:bg-slate-50/60 dark:hover:bg-slate-800/40"}`}>
                  ...
                  >
                    <div className={`w-4 h-4 mt-0.5 flex-shrink-0 rounded transition-colors ${expandedId === item.id ? "text-emerald-500" : "text-slate-500 group-hover:text-slate-500"}`}>
                      {expandedId === item.id
