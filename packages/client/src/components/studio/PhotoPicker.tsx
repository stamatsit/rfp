          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {!isLoading && photos.length > 0 && (
                <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight">{photos.length} photo{photos.length !== 1 ? "s" : ""} available</p>
              )}
            </div>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
              <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-700/60 flex items-center justify-center">
                <Image className="w-5 h-5 text-slate-500 dark:text-slate-400" />
              </div>
              <div>
                <p className="text-sm font-medium text-slate-600 dark:text-slate-300">No photos found</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{search ? "Try a different search term" : "No approved photos in the library"}</p>
              </div>
            </div>
          ) : (
