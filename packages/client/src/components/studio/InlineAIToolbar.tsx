...
import { Sparkles, Wand2, Minimize2, Maximize2, Type, Check, X, ChevronDown, Loader2, CornerDownLeft, RotateCcw, ArrowLeft } from "lucide-react"
...
const QUICK_ACTIONS: QuickAction[] = [
  { id: "rewrite",  label: "Rewrite",      icon: Wand2,     description: "Rewrite differently" },
  ...
  { id: "grammar",  label: "Fix Grammar",  icon: Type,      description: "Fix grammar & spelling" },
]
...
export function InlineAIToolbar({ selectedText, position, documentContext, onApply, onClose }: InlineAIToolbarProps) {
  ...
  const [lastAction, setLastAction] = useState<{ action: ActionId; custom?: string } | null>(null)
  ...
  const runAction = useCallback(async (action: ActionId, customInstruction?: string) => {
    setLastAction({ action, custom: customInstruction })
    setMode("streaming")
    setStreamedText("")
    setIsStreaming(true)
    setShowToneMenu(false)
  ...
  const handleRetry = useCallback(() => {
    if (lastAction) {
      runAction(lastAction.action, lastAction.custom)
    }
  }, [lastAction, runAction])

  // Calculate position — clamp both horizontal and vertical to viewport
  const toolbarStyle: React.CSSProperties = {
    top: Math.max(8, Math.min(position.top, window.innerHeight - 460)),
    left: Math.max(8, Math.min(position.left, window.innerWidth - 416)),
  }
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-slate-800 rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.12),0_0_0_1px_rgba(0,0,0,0.04)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.4),0_0_0_1px_rgba(255,255,255,0.06)] overflow-hidden"
        style={{ minWidth: 300, maxWidth: 400 }}
      >
        {/* Header — lighter treatment, no heavy gradient */}
        <div className="flex items-center justify-between px-3 py-2 border-b border-slate-100 dark:border-slate-700/60 bg-slate-50/80 dark:bg-slate-900/50">
          <div className="flex items-center gap-1.5">
            <div className="w-5 h-5 rounded-md flex items-center justify-center bg-gradient-to-br from-emerald-400 to-teal-500 shadow-sm">
              <Sparkles className="w-2.5 h-2.5 text-white" />
            </div>
            <span className="text-[11px] font-semibold text-slate-700 dark:text-slate-200">AI Edit</span>
          </div>
          <button
            onClick={onClose}
        ...
        {mode === "actions" && (
          <div className="p-2">
            ...
            {/* Tone dropdown */}
            <div className="relative mt-1" ref={toneMenuRef}>
              <button
            ...
            <div className="flex items-center gap-1.5 mt-2.5">
              {mode === "result" ? (
                <>
                  ...
                  <button
                    onClick={() => setMode("actions")}
                    className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors ml-auto"
                  >
                    <ArrowLeft className="w-3 h-3" />
                    Try another
                  </button>
